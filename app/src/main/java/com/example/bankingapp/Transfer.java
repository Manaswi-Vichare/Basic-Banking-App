...
public class Transfer extends AppCompatActivity {
    ...
    public void onTransferClick(final View view){
        ...
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("Confirm", (dialog, which) -> {
                    String senderBal="";
                    @SuppressLint("Recycle") Cursor c=myDataBase.rawQuery("SELECT * FROM customers WHERE custid='"+senderID+"'",null);
                    if(c.moveToFirst()){
                    ...
                    if(Double.parseDouble(senderBal)<Double.parseDouble(amt)){
                        ...
                        Toast.makeText(Transfer.this, "Transaction Failed due to insufficient balance", Toast.LENGTH_LONG).show();
                        finish();
                    }
                    else {
                        ...
                        arrow1.animate().rotationBy(180).setDuration(1000);
                        arrow2.animate().rotationBy(180).setDuration(1000);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        amtStr = findViewById(R.id.amtView);
        transferB = findViewById(R.id.transferButton);
        arrow1.animate().rotationBy(360).setDuration(1000).withEndAction(() -> {
            arrow1.animate().rotationBy(180).translationXBy(500).translationYBy(740).setDuration(1000);
            text.animate().alpha(1).setDuration(1000);
            amtStr.animate().alpha(1).setDuration(1000);
            transferB.animate().alpha(1).setDuration(1000);
        });
        arrow2.animate().rotationBy(360).setDuration(1000).withEndAction(() -> arrow2.animate().rotationBy(180).translationXBy(-500).translationYBy(-750).setDuration(1000));
